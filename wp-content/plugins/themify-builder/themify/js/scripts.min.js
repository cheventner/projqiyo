...
d=c.val().split("-"),e=c.data("range")?!0:!1;b.slider({range:e,min:c.data("min"),max:c.data("max"),values:d,slide:function(a,b){b.values[0]!=b.values[1]&&e?c.val(b.values[0]+"-"+b.values[1]):c.val(b.values[0])}})});void 0!==window.wp&&void 0!==window.wp.media&&"undefined"!==typeof wp.media.featuredImage&&(a("body").on("themify_metabox_lib_selected",function(a,c,d,e){c.data("submit").featured&&wp.media.featuredImage.set(d.id)}).on("themify_plupload_selected",function(a,c,d){c.data("featured")&&wp.media.featuredImage.set(d.id)}).on("themify_wp_media_featured_image_set",
function(b,c,d){b=a("#post_image").closest(".themify_field_row");-1==c?b.find(".themify_featimg_remove a").click():themifyMediaLib.setPreviewIcon(b,a(d).find("#set-post-thumbnail img").attr("src"))}),a("#post_image").closest(".themify_field_row").find(".themify_featimg_remove a").click(function(){a("#remove-post-thumbnail").click()}),wp.media.featuredImage.set=function(b){var c=wp.media.view.settings;c.post.featuredImageId=b;wp.media.post("get-post-thumbnail-html",{post_id:c.post.id,thumbnail_id:c.post.featuredImageId,
_wpnonce:c.post.nonce}).done(function(c){"0"==c?window.alert(window.setPostThumbnailL10n.error):(a(".inside","#postimagediv").html(c),a("body").triggerHandler("themify_wp_media_featured_image_set",[b,c]))})});m();y();K()});a(function(){var e=a("#themify .preview-icon");e.length&&e.each(function(){var e=a(this),f=e.attr("title");e.removeAttr("title").append('<span class="tm-option-title-fw">'+f+"</span>")})})})(jQuery);
