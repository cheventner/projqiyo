...
function(b,d){if(!a.data("confirm")||confirm(a.data("confirm")))b.refresh(),b.start(),c(".tb_alert").addClass("busy").fadeIn(800)});b.bind("Error",function(a,b){c(".prompt-box .show-login").hide();c(".prompt-box .show-error").show();if(-600==b.code){var d=themify_lang.filesize_error;var e=themify_lang.filesize_error_fix}0<c(".prompt-box .show-error").length&&(c(".prompt-box .show-error").html('<p class="prompt-error">'+d+"</p>"),e&&c(".prompt-box .show-error").append("<p>"+e+"</p>"));c(".overlay, .prompt-box").fadeIn(500)});
b.bind("FileUploaded",function(b,g,h){b=JSON.parse(h.response);var k="",k="200"!=h.status||b.error?"error":"done";c(".tb_alert").removeClass("busy").addClass(k).delay(800).fadeOut(800,function(){c(this).removeClass(k)});if(b.error)c(".prompt-box .show-login").hide(),c(".prompt-box .show-error").show(),0<c(".prompt-box .show-error").length&&(c(".prompt-box .show-error").html('<p class="prompt-error">'+b.error+"</p>"),c(".prompt-box .show-error").append("<p>"+themify_lang.enable_zip_upload+"</p>")),
c(".overlay, .prompt-box").fadeIn(500);else{c("#"+g.id).fadeOut();h=b.file;g=b.url;var l=b.type;"zip"===l||"rar"===l||"plain"===l?window.location=location.href.replace(location.hash,""):c("#"+d).val(g);"undefined"!==typeof b.thumb&&(themifyMediaLib.setPreviewIcon(a.closest(".themify_field, .themify_field_row"),b.thumb),c("body").trigger("themify_plupload_selected",[a,b]));e&&(c("#"+d).closest("fieldset").children(".preset").find("img").removeClass("selected"),b=g.replace(/^.*[\\\/]/,""),b=c('<a href="#" title="'+
b+'"><span title="'+h+'"></span><img src="'+g+'" alt="'+g+'" class="backgroundThumb selected" /></a>').css("display","inline-block"),c("#"+d).closest("fieldset").children(".preset").append(b));f&&c("#"+d+"-preview").attr("src",g)}})}jQuery(document).ready(function(a){var d=a(".plupload-upload-uic");0<d.length&&d.each(function(){themify_create_pluploader(a(this))})});
