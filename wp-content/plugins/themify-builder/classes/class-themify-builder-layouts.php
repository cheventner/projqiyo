...
if( !class_exists( 'Themify_Builder_Layouts' ) ) {
	...
	class Themify_Builder_Layouts{
		 ...
		 * @var object $layout .
		 */
		public static $layout_slug='tbuilder_layout';
		 ...
		 */
		public static $layout_part_slug='tbuilder_layout_part';
		...
		public function cleanup_builtin_layouts() {
			...
			wp_reset_postdata();
			$_key='themify_builder_cleanup_builtin_layouts';
			delete_option($_key);
	...
	class Themify_Builder_Layouts_Provider_Custom extends Themify_Builder_Layouts_Provider
	{
		...
		public function get_layouts() {
			...
			while ( $posts->have_posts() ) {
				...
				if ( empty( $url ) ) {
					$url = THEMIFY_BUILDER_URI . '/img/image-placeholder.jpg';
				}
				$layouts[] = array(
					'title' => get_the_title(),
					'slug' => $post->post_name,
