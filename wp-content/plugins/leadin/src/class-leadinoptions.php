...
class LeadinOptions {
	...
	const WP012_CONTROL           = 'wp-012-control';
	const WP012_VARIANT           = 'wp-012-variant';
	const GROUP_PARAMETER_OPTIONS = array( self::WP012_CONTROL, self::WP012_VARIANT );
	 ...
	 */
	public static function get_experiment_option() {
		return self::get_option( self::EXPERIMENT );
	}

	/**
	 * Return the time since the last connection on the plugin
	 */
	public static function get_review_option() {
		return self::get_option( self::REVIEW_TIME );
	}

	/**
	 * Set a random `experiment` option.
	 */
	public static function set_experiment_option() {
		if ( ! self::get_option( 'experiment' ) ) {
			add_option( self::EXPERIMENT, self::GROUP_PARAMETER_OPTIONS[ rand( 0, 1 ) ] );
		}
	}

	/**
	 * Set the time counter to show the review notice to now
	 */
	public static function set_review_option() {
		$date = new DateTime();
		if ( empty( self::get_review_option() ) ) {
			add_option( self::REVIEW_TIME, $date->getTimestamp() );
		}
	}
	 ...
	 * Set the time counter to show the review notice to now
	 */
	public static function reset_review_option() {
		$date = new DateTime();
		update_option( self::REVIEW_TIME, $date->getTimestamp() );
	}
}
